...
class TestHighPassFilter(unittest.TestCase):
    ...
    def test_highpass_rlc_series(self):
        cutoff_frequency = 1000  # Hz
        quality_factor = 0.707
        resistance = 1000  # Ohms

        result = self.filter.highpass_rlc_series(
        ...
        print("High-pass RLC Series Test:", result)

    def test_highpass_rlc_parallel(self):
        cutoff_frequency = 1000  # Hz
        quality_factor = 0.707
        resistance = 1000  # Ohms

        result = self.filter.highpass_rlc_parallel(
            cutoff_frequency, quality_factor, resistance=resistance
        )
        self.assertIn("L", result)
        self.assertIn("C", result)
        ...
        self.assertAlmostEqual(result["C"], 1.59e-7, places=3)
        print("High-pass RLC Parallel Test:", result)
    ...
    def test_exceptions(self):
        ...
        with self.assertRaises(ValueError):
            self.filter.highpass_rlc_series(1000, 0.707)

        # Test exception for missing resistance in RLC Parallel
        with self.assertRaises(ValueError):
            self.filter.highpass_rlc_parallel(1000, 0.707)
