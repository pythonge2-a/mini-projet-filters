...
class TestBesselFilters(unittest.TestCase):
    ...
    def test_first_order_lowpass(self):
        ...
        _, stages = self.lowpass_instance.components(
            order=order, cutoff_freq=self.cutoff_freq, r_vals=r_vals
        )
    ...
    def test_second_order_lowpass(self):
        ...
        self.assertAlmostEqual(
            ...
            c_vals[0],
            delta=self.tolerance,
            msg=f"Erreur pour R1 : {stages[0]['params']['C1']} != {c_vals[0]}",
        )
        self.assertAlmostEqual(
            stages[0]["params"]["C2"],
            c_vals[1],
            delta=self.tolerance,
            msg=f"Erreur pour R2 : {stages[0]['params']['C2']} != {c_vals[1]}",
        )
    ...
    def test_fourth_order_lowpass(self):
        ...
        # Résultats obtenus via la fonction
        _, stages = self.lowpass_instance.components(
            order=order, cutoff_freq=self.cutoff_freq, r_vals=r_vals, c_vals=c_vals
        )
        ...
        self.assertAlmostEqual(
            ...
            c_vals[0],
            delta=self.tolerance,
            msg=f"Erreur pour C1 : {stages[0]['params']['C1']} != {c_vals[0]}",
        )
        self.assertAlmostEqual(
            stages[0]["params"]["C2"],
            c_vals[1],
            delta=self.tolerance,
            msg=f"Erreur pour C2 : {stages[0]['params']['C2']} != {c_vals[1]}",
        )
        self.assertAlmostEqual(
            stages[0]["params"]["C1"],
            c_vals[2],
            delta=self.tolerance,
            msg=f"Erreur pour C1 : {stages[0]['params']['C1']} != {c_vals[2]}",
        )
        ...
        self.assertAlmostEqual(
            ...
            c_vals[3],
            delta=self.tolerance,
            msg=f"Erreur pour C2 : {stages[0]['params']['C2']} != {c_vals[3]}",
        )
    ...
    def test_first_order_highpass(self):
        ...
        # Résultats obtenus via la fonction
        _, stages = self.highpass_instance.components(
            order=order, cutoff_freq=self.cutoff_freq, r_vals=r_vals, c_vals=c_vals
        )
        ...
        self.assertAlmostEqual(
            ...
            r_vals[0],
            delta=self.tolerance,
            msg=f"Erreur pour C : {stages[0]['params']['R']} != {r_vals[0]}",
        )
    ...
    def test_second_order_highpass(self):
        ...
        self.assertAlmostEqual(
            ...
            r_vals[0],
            delta=self.tolerance,
            msg=f"Erreur pour R1 : {stages[0]['params']['R1']} != {r_vals[0]}",
        )
        self.assertAlmostEqual(
            stages[0]["params"]["R2"],
            r_vals[1],
            delta=self.tolerance,
            msg=f"Erreur pour R2 : {stages[0]['params']['R2']} != {r_vals[1]}",
        )
